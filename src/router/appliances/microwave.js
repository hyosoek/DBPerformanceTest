...
router.get("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        client.connect()
        const sql1 = `SELECT 
                        a.longitude,a.latitude,b.energy,b.model_name
                    FROM
                        account a 
                    JOIN 
                        microwave b 
        ...
        if(row1.length != 1){
            err = new Error()
            err.status = 403
            err.message = "Invalid data on account or microwave key!"
            throw err
        }
        ...
        console.log("GET /appliance/microwave", err.message)
        next(err)
    } finally{
...
router.post("/",auth.authCheck,async(req,res,next) =>{
    const {energy,modelname} = req.body;
    const result = {
        "success" : false,
        "message" : ""
    }
    ...
    try{
        ...
        const sql = `INSERT INTO 
                        microwave (account_id,energy,model_name) 
                    VALUES 
                        ($1,$2,$3);` //여기서 duplicate 체크
        const values = [req.decoded.id,energy,modelname]
        ...
        console.log("POST /appliance/microwave", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  
...
router.delete("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        console.log("DELETE /appliance/microwave", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  

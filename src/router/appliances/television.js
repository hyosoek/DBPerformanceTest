...
router.get("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        if(row1.length != 1){
            err = new Error()
            err.status = 403
            err.message = "Invalid data on account or television key!"
            throw err
        }
        ...
        console.log("GET /appliance/television", err.message)
        next(err)
    } finally{
...
router.post("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        const sql = `INSERT INTO 
                        television (account_id,energy,co2,model_name) 
                    VALUES 
        ...
        console.log("POST /appliance/television", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  
...
router.delete("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        console.log("DELETE /appliance/television", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  

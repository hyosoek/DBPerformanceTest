...
router.get("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        if(row1.length != 1){
            err = new Error()
            err.status = 403
            err.message = "Invalid data on account or dryer key!"
            throw err
        }
        ...
        console.log("GET /appliance/dryer", err.message)
        next(err)
    } finally{
...
router.post("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        const sql = `INSERT INTO 
                        dryer (account_id,energy,co2,model_name) 
                    VALUES 
        ...
        console.log("POST /appliance/dryer", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  
...
router.delete("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        console.log("DELETE /appliance/dryer", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  

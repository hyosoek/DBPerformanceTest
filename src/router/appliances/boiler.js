...
router.get("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        client.connect()
        const sql1 = `SELECT 
                        a.longitude,a.latitude,b.efficiency,b.gas_consumption,b.output,b.model_name
                    FROM
                        account a 
                    JOIN 
                        boiler b 
        ...
        if(row1.length != 1){
            err = new Error()
            err.status = 403
            err.message = "Invalid data on account or boiler key!"
            throw err
        }
        ...
        for(let i = maxDivideRangeCount;i >= 0; i--){ //주어진 범위에서 전체 범위까지 몇번 쪼갤것?
            ...
            const sql2 = `SELECT efficiency as data
                        FROM (
                            SELECT 
                                b.efficiency,
                                ROW_NUMBER()
        ...
        const relativeData = await relativeTier.getRelativeDescTier(nearUserData,row1[0].efficiency)
        ...
        console.log("GET /appliance/boiler", err.message)
        next(err)
    } finally{
...
router.post("/",auth.authCheck,async(req,res,next) =>{
    const {efficiency,gas_consumption,output,modelname} = req.body;
    const result = {
        "success" : false,
        "message" : ""
    }
    let client = null;
    try{
        inputCheck(efficiency).isEmpty().isFinite()
        inputCheck(gas_consumption).isEmpty().isFinite()
        inputCheck(output).isEmpty().isFinite()
        inputCheck(modelname).isEmpty().isMinSize(4).isMaxSize(50)

        client = new Client(db.pgConnect)
        client.connect()
        const sql = `INSERT INTO 
                        boiler (account_id,efficiency,gas_consumption,output,model_name) 
                    VALUES 
                        ($1,$2,$3,$4,$5);` //여기서 duplicate 체크
        const values = [req.decoded.id,efficiency,gas_consumption,output,modelname]
        ...
        console.log("POST /appliance/boiler", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  
...
router.delete("/",auth.authCheck,async(req,res,next) =>{
    ...
    try{
        ...
        console.log("DELETE /appliance/boiler", err.message) // 이건 해주는게 맞음
        next(err)
    } finally{
        if(client) client.end()
    }  
